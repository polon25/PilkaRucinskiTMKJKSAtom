...
public class Options extends JFrame { //by Jacek Piłka
	...
	public Options(){
		...
		leftPanelA.add(new JLabel("Kształt próbki"));
		leftPanelA.add(new JLabel());
		
		rightPanelA.setLayout(new GridLayout(4,1));
		rightPanelA.add(new JLabel());
		rightPanelA.add(new JLabel("Materiał otaczający"));
		...
		String stringShapes[]={"Kula", "Sześcian"};
		final JComboBox shapes = new JComboBox(stringShapes);
		String stringmaterials[]={"Prónia", "Woda"};
		final JComboBox materials = new JComboBox(stringmaterials);
		
		final JTextField mass = new JTextField("Podaj masa próbki");
		...
		ActionListener listener = new ActionListener() {
		    public void actionPerformed(ActionEvent e) {
		    	if(e.getSource()==elements){
	    			...
	    			else if(shapes.getSelectedItem()=="Sześcian"){
	    				shapeText="Krawędź [m]";
	    			}
